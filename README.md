...
- [Current Progress](#current-progress)
  ...
  - [🔧 Tool Calling](#-tool-calling-in-langchain)
...
- **Invoke external tools**: It can call web search engines like DuckDuckGo to gather real-time data and use it to answer user questions.
- **Interact with language models**: The agent integrates with Azure’s GPT-4, enabling it to generate natural language responses based on the provided input.
- **Handle complex tasks**: It combines the power of language models with external searches and can be customized to handle a wide variety of tasks based on user needs.

#### What is a ReAct Agent?

A **ReAct Agent** is a specific type of agent in Langchain that follows a "reasoning" and "acting" paradigm. It combines **reasoning** (thinking or analyzing) and **acting** (taking actions based on that reasoning) in a sequential and structured way. The agent does the following:

1. **Reasoning**: It analyzes the input and formulates a plan or response based on its understanding.
2. **Acting**: It then performs actions such as invoking external tools (e.g., DuckDuckGo search) or interacting with APIs to gather real-time information or perform specific tasks.
3. **Dynamic Decision-Making**: ReAct agents can adjust their reasoning and actions based on the data they retrieve, making them more flexible and intelligent than a traditional rule-based agent.

In this repository, a ReAct agent is created using the **Langchain** framework with an integration to **Azure GPT-4** for natural language processing and **DuckDuckGo** for real-time search functionality.

The ReAct agent is initialized by pulling a pre-defined prompt from Langchain’s hub. This prompt dictates how the agent reasons and acts. In the provided code, the agent executes tasks like searching for real-time information and responding to queries in a conversational manner.

### Example Usage of the ReAct Agent

The agent can be queried with questions like:
...
```
This will:
- Invoke the reasoning process.
- Call DuckDuckGo to search for relevant information on how to travel from Delhi to Goa.
- Provide a response generated by GPT-4, possibly combining its knowledge with real-time data from the web.
